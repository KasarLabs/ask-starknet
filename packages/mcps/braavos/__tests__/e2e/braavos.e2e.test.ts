...
import { CreateBraavosAccount } from '../../src/tools/createAccount.js';
import { DeployBraavosAccount } from '../../src/tools/deployAccount.js';
...
describe('Braavos E2E Tests', () => {
  beforeAll(async () => {
    ...
    const createAccountResult = await CreateBraavosAccount();

    if (createAccountResult.status !== 'success' || !createAccountResult.data) {
      throw new Error(
        `Failed to create Braavos account: ${createAccountResult.error || 'Unknown error'}`
      );
    }
  ...
  describe('Create Account + Transfer STRK scenario', () => {
    it('should create Braavos account successfully', () => {
    ...
    it('should transfer 0.005 STRK to the created account and verify balance', async () => {
      ...
      const deployResult = await DeployBraavosAccount(onchainRead, {
      ...
      if (deployResult.status === 'success' && deployResult.data) {
        ...
        expect(deployData.wallet).toBe('Braavos');
      }
