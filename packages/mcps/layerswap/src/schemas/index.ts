...
export const getNetworksSchema = z.object({
  network_types: z.array(z.string()).optional().describe('Filter networks by type. Possible values: evm, starknet, solana, cosmos, starkex, zksynclite, ton, paradex, tron, fuel, bitcoin, hyperliquid'),
});

// Get Sources
export const getSourcesSchema = z.object({
  destination: z.string().optional().describe('Destination network name - use the network name property (e.g., ETHEREUM_MAINNET, BASE_MAINNET)'),
  asset: z.string().optional().describe('Asset symbol - use the token symbol property (e.g., ETH)'),
});

// Get Destinations
export const getDestinationsSchema = z.object({
  source: z.string().optional().describe('Source network name - use the network name property (e.g., ETHEREUM_MAINNET, BASE_MAINNET)'),
  asset: z.string().optional().describe('Asset symbol - use the token symbol property (e.g., ETH)'),
});

// Get Swap Route Limits
export const getSwapRouteLimitsSchema = z.object({
  source_network: z.string().describe('Source network name - use the network name property (e.g., ETHEREUM_MAINNET, BASE_MAINNET)'),
  source_token: z.string().describe('Source token symbol - use the token symbol property (e.g., ETH)'),
  destination_network: z.string().describe('Destination network name - use the network name property (e.g., ETHEREUM_MAINNET, BASE_MAINNET)'),
  destination_token: z.string().describe('Destination token symbol - use the token symbol property (e.g., ETH)'),
  use_deposit_address: z.boolean().optional().describe('Whether to use deposit address'),
...
export const getQuoteSchema = z.object({
  ...
  slippage: z.string().optional().describe('Slippage tolerance in percentage format (e.g., 0.1 = 10%)'),
  use_deposit_address: z.boolean().optional().describe('Whether to use deposit address'),
  refuel: z.boolean().optional().describe('Whether to include refuel'),
});
...
export const createSwapSchema = z.object({
  source: z.string().describe('Source network name - use the network name property (e.g., ETHEREUM_MAINNET, BASE_MAINNET)'),
  destination: z.string().describe('Destination network name - use the network name property (e.g., ETHEREUM_MAINNET, BASE_MAINNET)'),
  source_asset: z.string().describe('Source asset symbol - use the token symbol property (e.g., ETH)'),
  destination_asset: z.string().optional().describe('Destination asset symbol - use the token symbol property (e.g., ETH)'),
